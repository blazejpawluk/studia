...
int main() {
	...
	for (const auto &file : paths) {
		...
		for (int i = 0; i < cd.n; i++) {
			// if (i % 100 == 0) cout << "    " << i << "/" << cd.n << endl;
			cout << "    " << i << "/" << cd.n << endl;
			
			vector<int> randomPath = cd.randomPath();
			int steps = 0;
			pair<int, vector<pair<double,double>>> temp = cd.localSearch2(randomPath, steps);
		...
		summary << file << endl;
		summary << " >  avg path: " << (double)avgPath / cd.n << endl;
		summary << " > avg steps: " << (double)avgSteps / cd.n << endl << endl;
