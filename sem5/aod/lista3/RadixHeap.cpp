...
int main(int argc, char* argv[]) {
	vector<string> input = parseCommand(argc, argv, "radixheap");
	...
	if(input[2] == "ss") {
		...
		writeSS(input[3], input[4], "radixheap", G.n, G.m, G.minCost, G.maxCost, total/n);
	} else if(input[2] == "p2p") {
		...
		writeP2p(input[3], input[4], "radixheap", G.n, G.m, G.minCost, G.maxCost, results);
	}
}