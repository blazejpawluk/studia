...
bool Graph::addEdge(int i, int j, int c) {
	...
	for (int idx = 0; idx < (int) E[i].size(); idx++)
		if (E[i][idx].to == j) return false;
...
void Graph::printFlow() {
	for (int i = 0; i < n; i++) {
		for (const auto &e : E[i]) if (e.capacity > 0) cout << "(" << i << "," << e.to << ")=" << e.flow << '/' << e.capacity << '\n';
	}
}

void Graph::printMatching() {
	for (int i = 1; i < n; i++) {
		for (const auto &e : E[i]) if (e.flow > 0 && e.to != n-1) cout << "(" << i << "," << e.to << ")\n";
	}
}