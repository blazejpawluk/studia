...
public class Bot {
	...
	private static Result minimax(int player, int depth, Result alpha, Result beta, boolean maximizing) {
		if (Board.winCheck(player)) return new Result(2000, depth); // pewna wygrana
		if (Board.winCheck(3 - player)) return new Result(-2000, depth); // pewna porazka
		if (Board.loseCheck(player)) return new Result(-1000, depth); // pewny brak wygranej
		if (Board.loseCheck(3 - player)) return new Result(1000, depth); // pewny brak porazki
