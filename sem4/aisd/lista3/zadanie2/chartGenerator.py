...
def convert_data_k(data):
    ...
    for n in ns:
        ...
        for record in data_n:
            ...
            temp[label]["comparisons"].append(record[3])
            temp[label]["swaps"].append(record[4])
...
def convert_data_no_k(data):
    ...
    for n in ns:
        ...
        data_n = data[mask]
        n_vals.append(n)
        comparisons.append(np.mean(data_n[:, 3]))
        swaps.append(np.mean(data_n[:, 4]))
...
def draw_plots(results_random, results_select):
    ...
    for label in labels:
        ...
        plt.title(f'Comparisons for {label}')
        plt.xlabel('n - array size')
        plt.ylabel('Average comparisons')
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(f"Comparisons_{label.replace('=', '').replace('/', '_')}.png")
        plt.close()
        ...
        plt.plot(results_select[label]['n'], results_select[label]['swaps'],
                 marker='o', linestyle='', markersize=3, label='select')
        plt.title(f'Swaps for {label}')
        plt.xlabel('n - array size')
        plt.ylabel('Average swaps')
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(f"Swaps_{label.replace('=', '').replace('/', '_')}.png")
        plt.close()
...
def draw_plots_no_k(n_random, comp_random, swaps_random, n_select, comp_select, swaps_select):
    ...
    plt.title('Global Comparisons (all k)')
    plt.xlabel('n - array size')
    plt.ylabel('Average comparisons')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("Global_Comparisons_all_k.png")
    plt.close()
    ...
    plt.title('Global Swaps (all k)')
    plt.xlabel('n - array size')
    plt.ylabel('Average swaps')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("Global_Swaps_all_k.png")
    plt.close()

def main():
    files = {
        'randomSelect': 'randomSelectTest.txt',
        'select': 'selectTest.txt'
    }
